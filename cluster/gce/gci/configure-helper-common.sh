...
function config-ip-firewall {
  echo "Configuring IP firewall rules" >> /var/log/master-init.log
  ...
  if iptables -w -L INPUT | grep "Chain INPUT (policy DROP)" > /dev/null; then
    echo "Add rules to accept all inbound TCP/UDP/ICMP packets" >> /var/log/master-init.log
  ...
  if iptables -w -L FORWARD | grep "Chain FORWARD (policy DROP)" > /dev/null; then
    echo "Add rules to accept all forwarded TCP/UDP/ICMP/SCTP packets" >> /var/log/master-init.log
  ...
  if [[ "${NON_MASQUERADE_CIDR:-}" == "0.0.0.0/0" ]]; then
    echo "Add rules for ip masquerade" >> /var/log/master-init.log
  ...
  if [[ "${ENABLE_METADATA_CONCEALMENT:-}" == "true" ]] && [[ ! "${METADATA_CONCEALMENT_NO_FIREWALL:-}" == "true" ]]; then
    echo "Add rule for metadata concealment" >> /var/log/master-init.log
...
function create-dirs {
  echo "Creating required directories" >> /var/log/master-init.log
...
function safe-block-symlink(){
  ...
  ln -s "${device}" "${sym}"
  echo "Created a symlink for SSD $ssd at ${sym}" >> /var/log/master-init.log
  chmod a+w "${sym}"
}
...
function get-or-generate-uuid(){
  ...
  echo "Generating or getting UUID from ${ssdmap}" >> /var/log/master-init.log
...
function safe-format-and-mount() {
  ...
  mkdir -p "${mountpoint}"
  echo "Mounting '${device}' at '${mountpoint}'" >> /var/log/master-init.log
...
function safe-bind-mount(){
  ...
  mkdir -p "${bindpoint}"
  echo "Binding '${mountpoint}' at '${bindpoint}'" >> /var/log/master-init.log
...
function mount-ext(){
  ...
  if [[ "${format}" == "fs" ]]; then
    ...
    echo "Disk format must be either fs or block, got ${format}" >> /var/log/master-init.log
  fi
}
...
function ensure-local-ssds() {
  ...
  for ssd in /dev/disk/by-id/google-local-ssd-*; do
    if [ -e "${ssd}" ]; then
      ...
      echo "No local SCSI SSD disks found." >> /var/log/master-init.log
    fi
  done
  ...
  for ssd in /dev/nvme*; do
    if [ -e "${ssd}" ]; then
      ...
      echo "No local NVMe SSD disks found." >> /var/log/master-init.log
    fi
  done
}
...
function mount-master-pd {
  ...
  echo "Mounted master-pd '${pd_path}' at '${mount_point}'" >> /var/log/master-init.log
...
function create-node-pki {
  echo "Creating node pki files" >> /var/log/master-init.log
...
function create-master-pki {
  echo "Creating master pki files" >> /var/log/master-init.log
...
function create-master-auth {
  echo "Creating master auth files" >> /var/log/master-init.log
...
function create-kubeconfig {
  local component=$1
  local token=$2
  echo "Creating kubeconfig file for component ${component}" >> /var/log/master-init.log
...
function create-kubelet-kubeconfig() {
  ...
  if [[ -z "${apiserver_address}" ]]; then
    echo "Must provide API server address to create Kubelet kubeconfig file!" >> /var/log/master-init.log
    exit 1
  fi
  if [[ "${CREATE_BOOTSTRAP_KUBECONFIG:-true}" == "true" ]]; then
    echo "Creating kubelet bootstrap-kubeconfig file" >> /var/log/master-init.log
...
EOF
    ...
    echo "Fetching kubelet kubeconfig file from metadata" >> /var/log/master-init.log
...
function create-kubeproxy-user-kubeconfig {
  echo "Creating kube-proxy user kubeconfig file" >> /var/log/master-init.log
...
function create-kubescheduler-policy-config {
  echo "Creating kube-scheduler policy config file" >> /var/log/master-init.log
...
function create-node-problem-detector-kubeconfig {
  local apiserver_address="${1}"
  if [[ -z "${apiserver_address}" ]]; then
    echo "Must provide API server address to create node-problem-detector kubeconfig file!" >> /var/log/master-init.log
    exit 1
  fi
  echo "Creating node-problem-detector kubeconfig file" >> /var/log/master-init.log
...
function assemble-docker-flags {
  echo "Assemble docker command line flags" >> /var/log/master-init.log
  ...
  if [[ -n "${DOCKER_REGISTRY_MIRROR_URL:-}" ]]; then
    echo "Enable docker registry mirror at: ${DOCKER_REGISTRY_MIRROR_URL}" >> /var/log/master-init.log
...
EOF
    ...
    echo "Docker command line is updated. Restart docker to pick it up" >> /var/log/master-init.log
...
function start-kubelet {
  echo "Start kubelet" >> /var/log/master-init.log
  ...
  echo "Using kubelet binary at ${kubelet_bin}" >> /var/log/master-init.log
...
function start-node-problem-detector {
  echo "Start node problem detector" >> /var/log/master-init.log
  local -r npd_bin="${KUBE_HOME}/bin/node-problem-detector"
  echo "Using node problem detector binary at ${npd_bin}" >> /var/log/master-init.log
...
function start-kube-proxy {
  echo "Start kube-proxy static pod" >> /var/log/master-init.log
...
function start-etcd-servers {
  echo "Start etcd pods" >> /var/log/master-init.log
  if [[ -d /etc/etcd ]]; then
    rm -rf /etc/etcd
  fi
...
function prepare-mounter-rootfs {
  echo "Prepare containerized mounter" >> /var/log/master-init.log
...
function start-kube-apiserver {
  echo "Start kubernetes api-server" >> /var/log/master-init.log
...
#  params+=" --address=127.0.0.1"
  ...
  echo "APISERVER_SERVICEGROUPID: ${APISERVER_SERVICEGROUPID:-0}" >> /var/log/master-init.log
  ...
  if [[ "${ENABLE_LEGACY_ABAC:-}" != "false" ]]; then
    echo "Warning: Enabling legacy ABAC policy. All service accounts will have superuser API access. Set ENABLE_LEGACY_ABAC=false to disable this." >> /var/log/master-init.log
  ...
  echo "apiserver params: ${params}" >> /var/log/master-init.log
  # Evaluate variables.
...
function start-kube-controller-manager {
  echo "Start kubernetes controller-manager" >> /var/log/master-init.log
...
function start-workload-controller-manager {
  ...
  echo "Start workload controller-manager" >> /var/log/master-init.log
...
function start-kube-scheduler {
  echo "Start kubernetes scheduler" >> /var/log/master-init.log
...
function start-cluster-autoscaler {
  if [[ "${ENABLE_CLUSTER_AUTOSCALER:-}" == "true" ]]; then
    echo "Start kubernetes cluster autoscaler" >> /var/log/master-init.log
...
function start-kube-addons {
  echo "Prepare kube-addons manifests and start kube addon manager" >> /var/log/master-init.log
  local -r src_dir="${KUBE_HOME}/kube-manifests/kubernetes/gci-trusty"
...
function start-lb-controller {
  ...
  if [[ "${ENABLE_L7_LOADBALANCING:-}" == "glbc" ]]; then
    echo "Start GCE L7 pod" >> /var/log/master-init.log
...
function start-flannel-ds {
  if [[ -f "${KUBE_HOME}/flannel/kube-flannel.yml" ]]; then
    echo "installing flannel ds" >> /var/log/master-init.log
    kubectl apply -f "${KUBE_HOME}/flannel/kube-flannel.yml"
  else 
    echo "failed to install flannel ds, cannot find required yaml file" >> /var/log/master-init.log
  fi
}
...
function setup-kubelet-dir {
    echo "Making /var/lib/kubelet executable for kubelet" >> /var/log/master-init.log
...
function gke-master-start {
  if [[ -e "${KUBE_HOME}/bin/gke-internal-configure-helper.sh" ]]; then
    echo "Running GKE internal configuration script" >> /var/log/master-init.log
...
function override-kubectl {
    echo "overriding kubectl" >> /var/log/master-init.log
...
function override-pv-recycler {
  if [[ -z "${PV_RECYCLER_OVERRIDE_TEMPLATE:-}" ]]; then
    echo "PV_RECYCLER_OVERRIDE_TEMPLATE is not set" >> /var/log/master-init.log
    exit 1
  fi
...
function setup-containerd {
  echo "Generate containerd config" >> /var/log/master-init.log
...
EOF
  ...
  echo "Restart containerd to load the config change" >> /var/log/master-init.log
