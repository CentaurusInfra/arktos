...
func NewReplicationControllerInformerWithMultiTenancy(client kubernetes.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers, tenant string) cache.SharedIndexInformer {
	return NewFilteredReplicationControllerInformerWithMultiTenancy(client, namespace, resyncPeriod, indexers, nil, tenant)
}
...
func NewFilteredReplicationControllerInformer(client kubernetes.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return NewFilteredReplicationControllerInformerWithMultiTenancy(client, namespace, resyncPeriod, indexers, tweakListOptions, "default")
}

func NewFilteredReplicationControllerInformerWithMultiTenancy(client kubernetes.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc, tenant string) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			...
			WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
				...
				return client.CoreV1().ReplicationControllersWithMultiTenancy(namespace, tenant).Watch(options)
			},
		},
...
func (f *replicationControllerInformer) defaultInformer(client kubernetes.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredReplicationControllerInformerWithMultiTenancy(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions, f.tenant)
}
