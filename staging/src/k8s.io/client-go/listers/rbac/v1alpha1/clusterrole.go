...
func (s clusterRoleTenantLister) List(selector labels.Selector) (ret []*v1alpha1.ClusterRole, err error) {
...
func (s clusterRoleTenantLister) Get(name string) (*v1alpha1.ClusterRole, error) {
	key := s.tenant + "/" + name
	if s.tenant == "system" {
		key = name
	}
