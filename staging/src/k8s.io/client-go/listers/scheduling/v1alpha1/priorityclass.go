...
func (s priorityClassTenantLister) List(selector labels.Selector) (ret []*v1alpha1.PriorityClass, err error) {
...
func (s priorityClassTenantLister) Get(name string) (*v1alpha1.PriorityClass, error) {
	key := s.tenant + "/" + name
	if s.tenant == "default" {
		key = name
	}
