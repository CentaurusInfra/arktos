...
func (s aPIServiceTenantLister) List(selector labels.Selector) (ret []*apiregistration.APIService, err error) {
...
func (s aPIServiceTenantLister) Get(name string) (*apiregistration.APIService, error) {
	key := s.tenant + "/" + name
	if s.tenant == "system" {
		key = name
	}
