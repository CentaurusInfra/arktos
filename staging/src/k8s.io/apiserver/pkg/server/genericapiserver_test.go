...
var system_user = &user.DefaultInfo{
	Name:   "fake-user",
...
func setUp(t *testing.T) (Config, *assert.Assertions) {
	...
	config.Authentication = AuthenticationInfo{Authenticator: InsecureSuperuser{}}
...
func newRequest(method, url string, body io.Reader) (*http.Request, error) {
	...
	return req.WithContext(request.WithUser(req.Context(), system_user)), nil
}
...
func TestCustomHandlerChain(t *testing.T) {
	...
	for i, test := range []Test{
		...
		var w io.Reader
		req, err := newRequest("GET", test.path, w)
		if err != nil {
...
func TestNotRestRoutesHaveAuth(t *testing.T) {
	...
	for _, test := range []struct {
		...
		req, _ := newRequest("GET", test.route, nil)
