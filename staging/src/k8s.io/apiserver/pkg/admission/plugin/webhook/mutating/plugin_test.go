...
func TestAdmit(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.Name, func(t *testing.T) {
			...
			ns := "webhook-test"
			client, informer := webhooktesting.NewFakeMutatingDataSource(metav1.TenantSystem, ns, tt.Webhooks, stopCh)
			...
			if tt.IsCRD {
				attr = webhooktesting.NewAttributeUnstructured(metav1.TenantSystem, ns, tt.AdditionalLabels, tt.IsDryRun)
			} else {
				attr = webhooktesting.NewAttribute(metav1.TenantSystem, ns, tt.AdditionalLabels, tt.IsDryRun)
			}
...
func TestAdmitCachedClient(t *testing.T) {
	...
	for _, tt := range webhooktesting.NewCachedClientTestcases(serverURL) {
		ns := "webhook-test"
		client, informer := webhooktesting.NewFakeMutatingDataSource(metav1.TenantSystem, ns, webhooktesting.ConvertToMutatingWebhooks(tt.Webhooks), stopCh)
		...
		err = wh.Admit(webhooktesting.NewAttribute(metav1.TenantSystem, ns, nil, false), objectInterfaces)
		if tt.ExpectAllow != (err == nil) {
