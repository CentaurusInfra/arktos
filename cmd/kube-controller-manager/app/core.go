...
func startNodeLifecycleController(ctx ControllerContext) (http.Handler, bool, error) {
	...
	if len(ctx.ComponentConfig.NodeLifecycleController.TenantPartitionKubeConfig) > 0 {
		tpAccessors, err = nodeutil.GetTenantPartitionManagersFromKubeConfig(ctx.ComponentConfig.NodeLifecycleController.TenantPartitionKubeConfig, "node-controller", ctx.Stop)
	} else {
