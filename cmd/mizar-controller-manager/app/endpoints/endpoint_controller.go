...
const (
	controllerKind = "endpoints"
	controllerFor  = "mizar_endpoints"
)
...
func NewObjectController(informer coreinformers.EndpointsInformer, kubeClient clientset.Interface) *ObjectController {
...
func (oc *ObjectController) handle(key string) error {
	...
	obj, err := oc.lister.EndpointsWithMultiTenancy(namespace, tenant).Get(name)
