...
const validityPeriod time.Duration = 365 * 100
...
// NewCloudGatewayCertDERandKey creates certificate and key for CloudGateway
func NewCloudGatewayCertDERandKey(cfg *certutil.Config) ([]byte, []byte, error) {
	...
	certDER, err := NewCertFromCa(cfg, caCert, serverKey.Public(), caKey, validityPeriod)
	if err != nil {
...
func NewCertFromCa(cfg *certutil.Config, caCert *x509.Certificate, serverKey crypto.PublicKey, caKey crypto.Signer, validityPeriod time.Duration) ([]byte, error) {
