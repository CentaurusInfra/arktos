...
func TestPriorityQueue_AddIfNotPresentWithMultiTenancy(t *testing.T) {
	q := NewPriorityQueue(nil, nil)
	addOrUpdateUnschedulablePod(q, &highPriNominatedPodWithMultiTenancy)
	q.AddIfNotPresent(&highPriNominatedPodWithMultiTenancy) // Must not add anything.
	q.AddIfNotPresent(&medPriorityPodWithMultiTenancy)
	q.AddIfNotPresent(&unschedulablePodWithMultiTenancy)
	expectedNominatedPods := &nominatedPodMap{
		nominatedPodToNode: map[types.UID]string{
			medPriorityPodWithMultiTenancy.UID:   "node1",
			unschedulablePodWithMultiTenancy.UID: "node1",
	...
	if getUnschedulablePod(q, &highPriNominatedPodWithMultiTenancy) != &highPriNominatedPodWithMultiTenancy {
		t.Errorf("Pod %v was not found in the unschedulableQ.", highPriNominatedPodWithMultiTenancy.Name)
	}
}
...
func TestPriorityQueue_AddUnschedulableIfNotPresent_BackoffWithMultiTenancy(t *testing.T) {
	...
	for i := 1; i < totalNum; i++ {
		...
		q.AddUnschedulableIfNotPresent(unschedulablePodWithMultiTenancy, oldCycle)
	}
...
func TestPriorityQueue_MoveAllToActiveQueueWithMultiTenancy(t *testing.T) {
	q := NewPriorityQueue(nil, nil)
	q.Add(&medPriorityPodWithMultiTenancy)
	addOrUpdateUnschedulablePod(q, &unschedulablePodWithMultiTenancy)
	addOrUpdateUnschedulablePod(q, &highPriorityPodWithMultiTenancy)
	q.MoveAllToActiveQueue()
	if q.activeQ.Len() != 3 {
...
func TestPriorityQueue_AssignedPodAddedWithMultiTenancy(t *testing.T) {
	...
	if getUnschedulablePod(q, &unschedulablePodWithMultiTenancy) == nil {
		t.Error("unschedulablePodWithMultiTenancy is not in the unschedulableQ.")
	}
}
