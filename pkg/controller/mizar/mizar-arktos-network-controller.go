...
func (c *MizarArktosNetworkController) Run(workers int, stopCh <-chan struct{}) {
	...
	<-stopCh
	klog.Info("shutting down network controller")
}
...
func (c *MizarArktosNetworkController) processNextWorkItem() bool {
	...
	utilruntime.HandleError(fmt.Errorf("Handle %v of key %q failed with %v", "arktos-network", key, err))
...
func (c *MizarArktosNetworkController) syncNetwork(eventKey KeyWithEventType) error {
	...
	tenant, _, name, err := cache.SplitMetaTenantNamespaceKey(key)
	if err != nil {
		return err
	}
	...
	case EventType_Create:
		err = c.processNetworkCreation(net)
	default:
...
func (c *MizarArktosNetworkController) processNetworkCreation(network *v1.Network) error {
	//skip update or create if type is not mizar or network status is ready
	if network.Spec.Type != mizarType || network.Status.Phase == v1.NetworkReady {
	...
	if code != CodeType_OK {
		return errors.New("Fail to update VPC with Arktos Network name on the Mizar cluster")
	}
