...
func Pod(pod *v1.Pod) string {
	return PodDescWithMultiTenancy(pod.Name, pod.Namespace, pod.Tenant, pod.UID)
}
...
// TODO: remove the above PodDesc() and change the following func to PodDesc()
// when the multi-tenancy change in kubelet is done.
// This function is just an interim form of PodDesc() during the process of multi-tenancy change in kubelet. 
func PodDescWithMultiTenancy(podName, podNamespace, podTenant string, podUID types.UID) string {
	// Use underscore as the delimiter because it is not allowed in pod name
	// (DNS subdomain format), while allowed in the container name format.
