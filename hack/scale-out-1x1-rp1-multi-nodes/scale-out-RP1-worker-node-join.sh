...
if [ $# -ne 1 ]; then
  echo "RP1 IP parameter is not set"
  exit 1
fi
...
for secret in kubelet.kubeconfig client-ca.crt kube-proxy.kubeconfig tenant-server-kube-proxy*.kubeconfig tenant-server-kubelet*.kubeconfig
do
  scp -i "$AWS_PRIVATE_KEY" ubuntu@$RP1_IP:$SECRET_DIR/$secret $SECRET_DIR/$secret
done
...
#IS_SCALE_OUT=true IS_RESOURCE_PARTITION=true API_HOST_IP_EXTERNAL=$RP1_IP CNIPLUGIN=flannel ./hack/arktos-worker-up.sh -O
make clean; IS_SCALE_OUT=true IS_RESOURCE_PARTITION=true API_HOST_IP_EXTERNAL=$RP1_IP CNIPLUGIN=flannel ./hack/arktos-worker-up.sh
